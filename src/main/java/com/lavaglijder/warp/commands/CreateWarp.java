...
public class CreateWarp implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if(args.length == 0) {
            p.sendMessage(ChatColor.translateAlternateColorCodes('&', messagesFile.getFile().get("createWarpUsage") + "") + "");
            return true;
        }

        if(warpAPI.warpExist(args[0])) {
            p.sendMessage(ChatColor.translateAlternateColorCodes('&', messagesFile.getFile().get("createWarpExist") + "") + "");
            return true;
        }
        Location l = p.getLocation();
        warpAPI.createWarp(args[0], l.getBlockX(), l.getBlockY(), l.getBlockZ(), l.getYaw(), l.getPitch(), l.getWorld());
        p.sendMessage(ChatColor.translateAlternateColorCodes('&', messagesFile.getFile().get("createWarpSuccess") + "").replace("(warp)", args[0]) + "");
